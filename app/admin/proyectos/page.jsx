      ...
      <div className="overflow-hidden rounded-[2rem] border border-neutral-200 bg-white shadow-card">
        {projects.length > 0 ? (
              ...
              <a key={project.id} href={`/admin/proyectos/${project.id}`} className="grid gap-4 p-5 transition hover:bg-neutral-50 lg:grid-cols-[1fr_auto] lg:items-center">
                <div>
                  <div className="flex flex-wrap gap-2">
                    ...
                    {project.featured ? <span className="rounded-full bg-[#F3FAEF] px-3 py-1 text-[11px] font-black uppercase tracking-[0.14em] text-lakuntza-greenDark">Home</span> : null}
                  </div>
          ...
          <div className="p-8 text-center">
            <h2 className="text-2xl font-black tracking-[-0.04em] text-neutral-950">Aún no hay trabajos publicados.</h2>
            ...
            <a href="/admin/proyectos/nuevo" className="mt-6 inline-flex rounded-2xl bg-neutral-950 px-5 py-3 text-sm font-black text-white">Crear primer trabajo</a>
          </div>
        )}
      </div>
    </AdminShell>
